...
drop table if exists bimining.member_churn_mkt_lose_label_001;
create table bimining.member_churn_mkt_lose_label_001 stored as orc as
...
where  pay_date>=regexp_replace(date_sub(from_unixtime(to_unix_timestamp('${hivevar:statis_date}','yyyymmdd'),'yyyy-mm-dd'),90),"-","") 
       ...
       and dept_cd in ('00016','00020','00025','00027','00029','00031'); --超市事业部代码

------------------------------step 3 针对未来3个月是否购买，为会员打流失标签---------------------	
drop table if exists bimining.member_churn_mkt_lose_label;
create table bimining.member_churn_mkt_lose_label stored as orc as
select t1.member_id,
        case when t2.member_id is null then '1' else '0' end as whether_churn,
		'${hivevar:statis_date}' as statis_date
from bimining.member_churn_mkt_lose_label_001 t1
left join (
             select distinct( member_id)
             from bimining.member_churn_order_info 
