...
drop table if exists bimining.member_churn_muying_twice_predict_001;
create table bimining.member_churn_muying_twice_predict_001 stored as orc as
select member_id,
		count(distinct pay_date)  as num_day --购买天数                                                                                           
from   bimining.member_churn_order_info                                                                                                
where  pay_date>=regexp_replace(date_sub(from_unixtime(to_unix_timestamp('${hivevar:statis_date}','yyyymmdd'),'yyyy-mm-dd'),1000),"-","")   
       and pay_date<='${hivevar:statis_date}'
       and dept_cd=00019 --母婴事业部代码
group by member_id                                                                                                                               
having count(distinct pay_date)>=2;  --购买天数2次及以上
			
------------------------------step 2 选取最近7个月有购买行为的会员---------------------	
drop table if exists bimining.member_churn_muying_twice_predict_002;
create table bimining.member_churn_muying_twice_predict_002 stored as orc as
select distinct member_id
from   bimining.member_churn_order_info
where  pay_date>=regexp_replace(date_sub(from_unixtime(to_unix_timestamp('${hivevar:statis_date}','yyyymmdd'),'yyyy-mm-dd'),210),"-","") 
       and pay_date<='${hivevar:statis_date}' 
       and dept_cd=00019;
	   
drop table if exists bimining.member_churn_muying_twice_predict_003;
create table bimining.member_churn_muying_twice_predict_003 stored as orc as
select t1.member_id
from  bimining.member_churn_muying_twice_predict_001 t1
inner join bimining.member_churn_muying_twice_predict_002 t2
on t1.member_id=t2.member_id;


------------------------------step 3 会员特征合并---------------------	
drop table if exists bimining.member_churn_muying_twice_predict_w;
create table bimining.member_churn_muying_twice_predict_w stored as orc as
select t1.member_id,
		t2.gender,
		t2.age_value,
...
from bimining.member_churn_muying_twice_predict_003 t1
