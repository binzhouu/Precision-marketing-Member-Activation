...
object CustChurnMkt{
	def main(args:Array[String]){
		val spark=SparkSession.builder()
					.appName("CustChurnMkt")
					.enableHiveSupport()
					.getOrCreate()
		import spark.implicits._
		//val dataDF1=spark.read.option("header","true").csv("E:/Users/wangxl/spark/CustChurn/mktData.csv")
		val dataDF1=spark.sql("select * from bimining.member_churn_mkt_train_w where gender is not null")
		...
		////////训练模型///////
		val classifier=new RandomForestClassifier().setImpurity("gini").setMaxDepth(15).setNumTrees(50).
		...
		//读入预测数据
		val preDF1=spark.sql("select * from bimining.member_churn_mkt_pre_w where gender is not null")
		...
		PreResult.select("member_id","predictedLabel","prob").write.insertInto("bimining.member_churn_predict_mkt")
