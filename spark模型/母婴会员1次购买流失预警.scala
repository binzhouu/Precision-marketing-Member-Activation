...
object CustChurnMuyingOnce{
	def main(args:Array[String]){
		val spark=SparkSession.builder()
					.appName("CustChurnMuyingOnce")
					.enableHiveSupport()
					.getOrCreate()
		import spark.implicits._
		//val dataDF1=spark.read.option("header","true").csv("E:/Users/wangxl/spark/CustChurn/muyingOnceData.csv")
		val dataDF1=spark.sql("select * from bimining.member_churn_muying_once_train_w where gender is not null")
		val dataDF2=dataDF1.drop("child_age")
		val dataDF=dataDF2.drop("reliability_score_age")
		...
		dataDF("reliability_grade_age").cast("String"),
		dataDF("accur_grade_age").cast("String"),
		...
		//reliability_grade_age one-hot编码
		val indexer5=new StringIndexer().setInputCol("reliability_grade_age").setOutputCol("reliabilityGradeAgeIndex").fit(encoded4)
		val indexed5=indexer5.transform(encoded4)
		val encoder5=new OneHotEncoder().setInputCol("reliabilityGradeAgeIndex").setOutputCol("reliabilityGradeAgeVec").setDropLast(false)
		val encoded5=encoder5.transform(indexed5)
		//accur_grade_age one-hot编码
		val indexer6=new StringIndexer().setInputCol("accur_grade_age").setOutputCol("accurGradeAgeIndex").fit(encoded5)
		val indexed6=indexer6.transform(encoded5)
		val encoder6=new OneHotEncoder().setInputCol("accurGradeAgeIndex").setOutputCol("accurGradeAgeVec").setDropLast(false)
		...
		"loyaltyVec","scorLabelVec","reliabilityGradeAgeVec",
		"accurGradeAgeVec","complaints_1y","complaints_6m","complaints_3m",
		...
		val vecDF=new VectorAssembler().setInputCols(colArray).setOutputCol("features").transform(encoded6)
		
		//对样本进行重采样，使得正负样本平衡
		//val sample_0=vecDF.where("whether_churn=0")
		//val n_0=sample_0.count()
		//val sample_1=vecDF.where("whether_churn=1.0")
		//val n_1=sample_1.count()
		//val n=n_0/n_1
		//val sample_1_1=sample_1.sample(true,1)
		//val data=sample_0.union(sample_1_1)
		val data=vecDF
		...
		////////训练模型///////
		val classifier=new RandomForestClassifier().setImpurity("gini").setMaxDepth(20).setNumTrees(35).
		...
		//读入预测数据
		//val preDF1=spark.read.option("header","true").csv("E:/Users/wangxl/spark/CustChurn/muyingOncePreData.csv")
		val preDF1=spark.sql("select * from bimining.member_churn_muying_once_predict_w where gender is not null")
		val preDF2=preDF1.drop("child_age")
		val preDF3=preDF2.drop("reliability_score_age")
		...
		preDF3("reliability_grade_age").cast("String"),
		preDF3("accur_grade_age").cast("String"),
		...
		//reliability_grade_age one-hot编码
		val indexerReliability=new StringIndexer().setInputCol("reliability_grade_age").setOutputCol("reliabilityGradeAgeIndex").fit(encodedScorLabel)
		val indexedReliability=indexerReliability.transform(encodedScorLabel)
		val encoderReliability=new OneHotEncoder().setInputCol("reliabilityGradeAgeIndex").setOutputCol("reliabilityGradeAgeVec").setDropLast(false)
		val encodedReliability=encoderReliability.transform(indexedReliability)
		//accur_grade_age one-hot编码
		val indexerAccur=new StringIndexer().setInputCol("accur_grade_age").setOutputCol("accurGradeAgeIndex").fit(encodedReliability)
		val indexedAccur=indexerAccur.transform(encodedReliability)
		val encoderAccur=new OneHotEncoder().setInputCol("accurGradeAgeIndex").setOutputCol("accurGradeAgeVec").setDropLast(false)
		val encodedAccur=encoderAccur.transform(indexedAccur)
		...
		"lgs_amnt_1w","gds_cmnt_1y","gds_cmnt_6m","gds_cmnt_3m","gds_cmnt_1m","gds_cmnt_3w",
		"gds_cmnt_2w") //"child_age","reliability_score_age",
		val vecDFPre=new VectorAssembler().setInputCols(colArrayPre).setOutputCol("features").transform(encodedAccur)
		//输出结果
		//model.toDebugString
		...
		PreResult.select("member_id","predictedLabel","prob").write.insertInto("bimining.member_churn_predict_muying_once")
		...
		result.select("whether_churn","label","prediction","predictedLabel","prob").show(100,false)
		//模型准确度评估
