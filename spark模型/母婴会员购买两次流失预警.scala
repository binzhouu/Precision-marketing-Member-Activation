...
object CustChurnMuyingTwice{
	def main(args:Array[String]){
		val spark=SparkSession.builder()
					.appName("CustChurnMuyingTwice")
					.enableHiveSupport()
					.getOrCreate()
		import spark.implicits._
		//val dataDF1=spark.read.option("header","true").csv("E:/Users/wangxl/spark/CustChurn/muyingTwiceData.csv")
		val dataDF1=spark.sql("select * from bimining.member_churn_muying_twice_train_w  where gender is not null")
		...
		//读入预测数据
		val preDF1=spark.sql("select * from bimining.member_churn_muying_twice_predict_w where gender is not null")
		...
		PreResult.select("member_id","predictedLabel","prob").write.insertInto("bimining.member_churn_predict_muying_twice")
